...
recover  trace sets traces maximizing $\probability_\theta
...
\centering  \begin{minipage}[t]{2.5cm}\centering\includegraphics[width = 2cm]{figures/expert-60-reduced.png}
  ...
  \begin{minipage}[t]{2.5cm}\includegraphics[width = 2cm]{figures/60-1-reduced.png}
    \subcaption{}
  \end{minipage}%
    \caption{(a): hand drawing. (b): Rendering of the trace our model infers for (a). (c): noisy rendering of (b).}\label{handDrawingExamples}
\end{wrapfigure}
...
For each drawing we annotated a ground truth trace and have the neurally guided SMC sampler
...
\begin{table}[H]
  \caption{Grammar over graphics programs. We allow loops (\texttt{for}) with conditionals (\texttt{if}), vertical/horizontal reflections (\texttt{reflect}), variables (Var) and affine transformations ($\mathbb{Z}$\texttt{ * }Var\texttt{ + }$\mathbb{Z}$).}\label{DSL}
...
program satisfying the specification.  Sketch requires that the
program space be \emph{finite}, which here means that the depth of the
...
\begin{table}[t]
\centering  \caption{Pairs of images either close together or far apart in different features spaces.  The symbol $\leftrightarrow$ points to the compared images. Program features base captures abstract notions like symmetry and repetition. Distance metric over images is $L_{\text{learned}}(\cdot|\cdot)$ (see Section~\ref{generalizingTheHandDrawings}).}\label{similarityMatrix}
  \begin{tabular}{lll}
    &\textbf{Close in program space}&\textbf{Far apart in program space}\\\\
    \textbf{Close in image space}&\begin{tabular}{l}
    ...
    \begin{tabular}{l}
      ...
      \end{tabular}&\\
    \textbf{Far apart in image space}&
    \begin{tabular}{l}
