...
rectangle(...)
    ...
    \draw[decoration = {brace,mirror,raise = 5pt},decorate,thick]
([xshift = 150,yshift = -15]trace.south) -- node[below = 6pt] {{\small Section~\ref{applicationsSection}: Applications}} ([xshift = 250,yshift = -15]trace.south);
  \end{tikzpicture}
...
another drawing command to add to the trace set; if it decides
...
trace set; see Tbl.~\ref{drawingCommandTable} for the drawing commands we currently model.  We predict the drawing
...
\clearpage\begin{wrapfigure}{R}{8cm}
  \centering
...
Given the target image, this baseline produces the trace set in one shot by
...
Given the DSL and a trace set $T$, we want to recover a program that both evaluates to $T$
...
always eventually finds the globally optimal solution.
...
\pagebreak\section{Applications of graphics program synthesis}\label{applicationsSection}
...
\begin{figure}[t]
  \begin{tabular}{c|c|c|}
    \multicolumn{1}{c}{}    &\multicolumn{1}{c}{\textbf{Close in program space}}&\multicolumn{1}{c}{\textbf{Far apart in program space}}\\\cline{2-3}
    \multicolumn{1}{c|}{}&&\\
    \multicolumn{1}{c|}{    \begin{tabular}{l}\textbf{Close in }\\ \textbf{image space} \end{tabular}}    &
    \begin{tabular}{l}
      \begin{minipage}{\similaritySize}\includegraphics[width = \similaritySize]{figures/expert-20-trim.png}\end{minipage}\begin{minipage}{\similarityArrowSize}$\leftrightarrow$\end{minipage}\begin{minipage}{\similaritySize}\includegraphics[width = \similaritySize]{figures/expert-21-trim.png}\end{minipage}\\\\
      \begin{minipage}{\similaritySize}\includegraphics[width = \similaritySize]{figures/expert-50-trim.png}\end{minipage}\begin{minipage}{\similarityArrowSize}$\leftrightarrow$\end{minipage}\begin{minipage}{\similaritySize}\includegraphics[width = \similaritySize]{figures/expert-2-trim.png}\end{minipage}
    \end{tabular} &
    \begin{tabular}{l}
    ...
    \multicolumn{1}{c|}{}&&\\
    \multicolumn{1}{c|}{    \begin{tabular}{c}\textbf{Far apart in }\\ \textbf{image space} \end{tabular}}    &        \begin{tabular}{l}
