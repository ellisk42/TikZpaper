...
\usepackage{iclr2018_conference,times}%\usepackage[final]{nips_2017}
...
\section{Further testing of the trace hypothesis}

Recall that we factored the graphics program synthesis problem into two components: (1) a perception-facing component, whose job is to go from perceptual input to a set of commands that must occur in the execution trace of the program (\textbf{trace set}); and (2) a program synthesis component, whose job is to infer a program whose execution trace contains those commands. This is a different approach from other recent program induction models (e.g.,~\cite{devlin2017robustfill,nps}), which regress directly from a program induction problem to the source code of the program. We motivated the usefulness of our factoring of the problem via The Trace Hypothesis.

\textbf{Experiment.} To test whether the trace hypothesis is necessary for our domain, we trained a model to regress directly from images to graphics programs. This baseline model, which we call the \emph{no-trace baseline}, was able to infer some simple programs, but failed completely on more sophisticated scenes.
...
\begin{figure}[h]\centering
  ...
  \caption{Top, white: synthetic rendering of a hand drawing. Bottom, black: output of best program found by no-trace baseline.}\label{noTrace}
  \end{figure}
