...
\begin{equation}
  \probability [T_n|T_{1:(n - 1)}]\propto \text{MLP}_{T_1,n}(a(f|T_{1:(n - 1)}) \otimes \bigotimes_{j < n} \text{oneHot}(T_j))
\end{equation}
...
\begin{equation}
  a(f|T_{1:(n - 1)}) = \text{AffineTransform}(f, \text{MLP}_{T_1,n}(\bigotimes_{j < n}\text{oneHot}(T_j)))
