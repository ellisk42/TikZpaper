...
Even with the high-level idea of a trace set, going from hand drawings to programs remains difficult. We address the following challenges:
(1) Inferring trace sets from noisy hand drawings requires design choices from the deep learning and computer vision toolkits (Section~\ref{neuralNetworkSection}); (2) Discovering good programs requires solving a difficult combinatorial search problem, because the programs are often long and complicated (e.g., 9 lines of code, with nested loops and conditionals). We give a generic framework for learning a \emph{search policy} that quickly guides program synthesizers toward the target programs.
