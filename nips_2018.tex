...
\newcommand{\indicator}{\mathds{I}} %{{\rm I\kern-.3em P}}
...
we interpret our two-step factoring  as a generative model where a latent program
 ...
 \begin{figure}[h]
  \centering\begin{tikzpicture}
    \node[ thick,anchor = west,inner sep=0pt,label={[yshift = 0.3cm]{\small Image}}](observation) at (0,0) {\includegraphics[width = 1.5cm]{figures/expert-39-trimmed.png}};
...
\end{lstlisting}};
    ...
    \node(trace)[draw,thin,fit = (traceImage) (traceSource), label = above:{{\small Drawing commands (Spec)}}] {};
    
    \node[draw, thick,anchor = west,inner sep=2pt,label=above:{\small Program}](program) at (6.3,0) {
      \begin{lstlisting}
for (j < 3)
for (i < 3)
if (...)
...
rectangle(...)
    \end{lstlisting}};
    \node[ultra thick,anchor = west,inner sep=0pt,label=below:{\small Extrapolation}](extrapolate) at (9.9,0.5) {\includegraphics[width = 1.3cm]{figures/39-extrapolated.png}};
%    \node[ultra thick,anchor = west,inner sep=0pt,label=below:{\small Similarity}](similarity) at (11.3,0) {\includegraphics[width = 1cm]{figures/expert-38-trim.png}};
    \node[ultra thick,anchor = west,text width = 1.3cm,inner sep=0pt](errors) at (10,-1.2) {\small Error correction};

    \draw[->, ultra thick] (trace.west) -- node[fill = white,rotate = -90] {{\small Rendering}} (observation.east);
    \draw[->, thick, red] ([xshift = -5]observation.south) to[out = -30,in = -150] node[midway,yshift = -6]{{\small Learning + Stochastic search}} ([xshift = -15]trace.south);
    \draw[->, ultra thick] (program.west) -- node[fill = white,rotate = -90] {{\small Execution}} (trace.east);
    \draw[->, thick, red] ([xshift = 10]trace.south) to[out = -10,in = -170] node[midway,yshift = -6]{{\small Learning + Program synthesis}} ([xshift = 10]program.south);
%    \draw[->, thick, red] (program.east) to[out = 80,in = 180] (errors.west);
%    \draw[->, thick] (program.east) to[out = 40,in = -230] (similarity.west);
    \draw[->, thick, red] (program.east) to[out = 60,in = 180] (extrapolate.west);

    \draw[decoration = {brace,mirror,raise = 5pt},decorate,thick]
    ([yshift = -15,xshift = -115]trace.south) -- node[below = 6pt] {{\small Section~\ref{neuralNetworkSection}: Image$\to$Spec}} ([yshift = -15,xshift = -5]trace.south);
    \draw[decoration = {brace,mirror,raise = 5pt},decorate,thick]
    ([xshift = 5,yshift = -15]trace.south) -- node[below = 6pt] {{\small Section~\ref{programSynthesisSection}: Spec$\to$Program}} ([xshift = 140,yshift = -15]trace.south);
    \draw[decoration = {brace,mirror,raise = 5pt},decorate,thick]
...
\begin{table}[h]
  ...
  \begin{tabular}{lll}
    \toprule Model&\begin{tabular}{c}
      Median \\search time
      \end{tabular}&Timeouts\\\midrule
    Sketch&274 sec&27\%\\
    Oracle&6 sec&2\%\\
    Learned policy&28 sec&2\%\\\bottomrule 
    \end{tabular}.  
  \end{minipage}\hfill%
  \begin{minipage}[c]{0.5\textwidth}
    \caption{How long does it typically take to synthesize a minimum cost program? Sketch: out-of-the-box performance of the Sketch~\citep{solar2008program} program synthesizer. Oracle:  upper bounds  the performance of any search policy. Learned policy: our bias-optimal learned search policy running.  %% $\infty = $ timeout. Red dashed line is median time.
          Learned policy evaluated using 20-fold cross validation.}\label{policyEvaluation}
    \end{minipage}
\end{figure} 
