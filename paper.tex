...
Although these program synthesizers do not \emph{need} any training data,
we show how to combine conventional machine learning with bias optimal search~\cite{schmidhuber2004optimal}
in order to synthesize complicated programs in only a few minutes.
...
\noindent \textbf{The Trace Hypothesis.} The execution trace of a program
is the correct liaison between high-level structured symbolic representations
and high-dimensional unstructured perceptual input.
...
\begin{figure}
  \begin{tikzpicture}
    ...
    \node[ultra thick,anchor = west,inner sep=0pt,label=below:{\small Trace}](trace) at (3,0) {\includegraphics[width = 1.5cm]{../TikZ/drawings/expert-39-parses/0.png}};
    
    \node[draw, thick,anchor = west,inner sep=0pt,label=below:{\small Program}](program) at (6,0) {
      \begin{lstlisting}
for (j < 3)
for (i < 3)
if (...)
...
rectangle(...)
    ...
    ([yshift = -15,xshift = -30]observation.south) -- node[below = 6pt] {Section~\ref{neuralNetworkSection}: Perception$\to$Trace} ([yshift = -15,xshift = -5]trace.south);
    \draw[decoration = {brace,mirror,raise = 5pt},decorate,thick]
...
([xshift = 150,yshift = -15]trace.south) -- node[below = 6pt] {Section~\ref{applicationsSection}: Applications} ([xshift = 250,yshift = -15]trace.south);
  \end{tikzpicture}
  \caption{Paper roadmap: Structured around the \emph{trace hypothesis}}\label{roadmap}
\end{figure}
...
Synthesizing programs from their execution traces is an old
idea~\cite{Summers:1977:MLP:321992.322002}.  This previous work
departs from our approach in the nature of the problem (synthesizing
recursive algorithms in Lisp vs. graphics programs in~\LaTeX) and the
specifics of the solution (hand-coded source-to-source transformations vs. neural networks
combined with general-purpose program synthesis techniques).
What they share is the use of an execution trace as a
stepping stone toward a full program.
...
\\  \bottomrule
\end{tabular}
\caption{The deep network in Fig.~\ref{architecture} predicts drawing commands, shown above.}
...
\begin{figure}[H]
  \begin{minipage}[t]{2.25cm}
    ...
    \subcaption{A recurrent network architecture}
  \end{minipage}
  \begin{minipage}[t]{2.25cm}
...
\begin{figure}[h]\centering
\vspace{-0.25cm}  \begin{minipage}{0.8\textwidth} 
...
we seek to synthesize simple graphics programs from their execution traces.
...
In practice, synthesis times vary from a few minutes to a a few hours,
...
A main impediment here to our use of these general techniques is
the prohibitively high cost of searching for programs.

\subsection{Learning a search policy for synthesizing programs}
...
that allocates $\probability_{\text{bias}}[\sigma ]$ of its time to testing $\sigma $.
...
Of course one could never construct such an Oracle,
...
\begin{figure}
  \includegraphics[width = 12cm]{figures/policyComparison.png}
  \caption{$\infty = $ timeout}\label{policyEvaluation}
  \end{figure}

This generalizes and strengthens previous work: Levin
search~\cite{levin1973universal,solomonoff1984optimum} \&
OOPS~\cite{schmidhuber2004optimal}: theoretically appealing but
impractical; fails to leverage either techniques from machine learning
or from program synthesis. More of a mathematical framing and less of
an algorithm.  DeepCoder~\cite{BalGauBroetal16}: in addition to
picking what sort of primitives can go into a program, we are picking
the details of the search algorithm as well as how deep the algorithms
should search.  Main difference is that we identify this interesting
trade-off between tractability and probability of getting a solution.
TerpreT~\cite{gaunt2016terpret}: suggests that constraint solvers outperform gradient-based search techniques like those used in differentiable neural computers.


Two intuitions: want a model that prefers sketches that are likely to
contain a valid solution, but at the same time prefers sketches that
are easy to search.  In general these two preferences are at odds with
one another.  For example, a sketch which allows very long programs
and allows lots of primitives is extremely likely to contain a
shortest program -- but might also take hours for the synthesizer to
search through.



\newcommand{\exampleProgramSize}{4.5cm}
\newcommand{\exampleTraceSize}{4cm}
\newcommand{\exampleDrawingSize}{2cm}

\begin{figure}
\hspace{-0.7cm}  \begin{tabular}{m{2.5cm}llc}
    \toprule
...
\end{minipage}&$\frac{9}{5} = 1.8\text{x}$\\\bottomrule
  \end{tabular}
  \caption{Example drawings (left), their ground truth traces (middle left), and programs synthesized from these traces (middle right). Compared to the trace the programs are more compressive (right: programs have fewer lines than traces) and automatically group together related drawing commands. Note the nested loops in the Ising model, special case conditionals for the HMM, combination of symmetry and iteration in the bottom figure, and affine transformation in the top figure.}\label{exampleSynthesisResults}
  \end{figure}

With this policy in hand we can now synthesize most programs in under a minute.
