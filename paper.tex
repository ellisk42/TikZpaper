...
learning to improve the wall clock speed of these synthesis algorithms by two orders of magnitude.
...
\begin{table}[h]\centering
  ...
  \\\bottomrule
  \end{tabular}
  \caption{Parameterization of different ways of posing the program synthesis problem. The policy learns to choose parameters likely to quickly yield a minimal cost program. In our notation, we write $\sigma $ to mean an assignment to each of these parameters.}\label{policyOutput}
  \end{table}
...
\text{where }  \sigma \in\text{\textsc{Best}}(T) &\text{ iff:  } \text{cost}(\text{program}(p)) = \min_{p\in \sigma \text{, s.t. }p\text{ evaluates to }T} \text{cost}(p) \nonumber
\end{align}
This loss is differentiable but nonconvex even if $\pi_{\theta}(\cdot |\cdot )^{-1}$ is convex.
We locally minimize it using gradient descent while annealing a softened minimum to the hard minimization equation~\ref{policyLoss}. See supplement for details.
...
With this policy in hand we can now synthesize most programs in under a minute. But why synthesize a graphics program,
