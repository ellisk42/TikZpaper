...
in order to synthesize programs over an order of magnitude faster than constraint-based synthesis techniques alone.
...
Our new contribution is an approach for
converting perceptual input to high-level programs.
...
Of course one could never construct this Oracle,
...
\centering  \includegraphics[width = 14cm]{figures/policyComparison.png}
  \caption{How long does it typically take to synthesize a minimum cost program? Sketch: out-of-the-box performance of the Sketch~\cite{solar2008program} program synthesizer. Oracle: baseline that always picks the easiest synthesis problem containing a minimum cost program. Learned policy: a bias-optimal learned search policy. Oracle is impossible to implement and is an upper bound on the performance of any search policy. $\infty = $ timeout. Red dashed line is median time.}\label{policyEvaluation}
  \end{figure}
...
the trade-off between tractable search and probability of successful
...
against gradient-based search techniques (e.g., a
DNC~\cite{graves2016hybrid}).  The TerpreT experiments help motivate
our use of constraint-based techniques.
...
that execution traces can be inferred from perceptual data,
