...
\begin{abstract}
  ...
  that explain an image.  From these deep net proposals we can use
  program synthesis techniques to recover a graphics
  program with constructs like variable bindings, iterative loops, or
  simple kinds of conditionals.~\remark{Can you (concisely) communicate the idea of treating the deep net proposal as an `execution trace?' It's a really neat intellectual framing that deserves to be foregrounded.} With a graphics program in hand,
  we can correct errors made by the deep network, extrapolate drawings, and~\remark{what else: `visual analogies' is vague. `Cluster drawings by use of similar high-level program structures?'}
...
\subcaption{}
  ...
  \begin{minipage}[t]{0.3\linewidth}
    ...
    \subcaption{}
  \end{minipage}
  \caption{(a): Model learns to convert hand drawings (top) into \LaTeX~(bottom). (b) Synthesizes high-level \emph{graphics program} from hand drawing.~\remark{Any objection to putting this on page bottom?}}\label{firstPageExamples}
  \end{figure}
...
stochastic search over execution traces.  Finally, we use program synthesis to recover the program from its
...
We designed this noise process to introduce the kinds of variations found in hand drawings (Figure~\ref{handDrawingExamples}).~\remark{Process should be described (in supplement)}
...
Our learned $L(\cdot|\cdot)$
is a convolutional network that we train to predict
the distance between two traces conditioned upon their renderings~\remark{The two $L$'s should have some kind of identifier e.g. a subscript to distinguish them}.
Formally we train our likelihood surrogate to approximate:
\begin{equation}
-\log L(\text{render}(T_1)|\text{render}(T_2))\approx |T_1 - T_2| + |T_2 - T_1|
  \end{equation}

\remark{What distance metric is this? Some kind of edit distance?}
...
\begin{figure}\centering
  ...
  \begin{minipage}[t]{0.3\textwidth}\includegraphics[width = 1.5cm]{figures/60-1-reduced.png}
    \subcaption{}
  \end{minipage}%
    \caption{(a): a hand drawing. (b): Rendering of the trace our model infers for (a). We can generalize to hand drawings like these because we train the model on images corrupted by a noise process designed to resemble the kind of noise introduced by hand drawings - see (c) for a noisy rendering of (b).}\ref{handDrawingExamples}
  \end{minipage}\hfill
  \begin{minipage}{0.45\textwidth}
...
\begin{figure}[H]
  ...
  \begin{minipage}[t]{2.25cm}
    ...
    \end{minipage}
  \caption{Example drawings above model outputs. See also Fig.~\ref{firstPageExamples}}\label{handDrawingExamples}~\remark{Showing `failure cases' (the last three above) out-of-context seems not great. Perhaps remind people in the caption that stochastic search can help correct some of these issues?}
\end{figure}
...
For 55\% of the drawings the Top-1 most likely sample exactly matches the
...
We also penalize using more distant constants~\remark{awkward},
which encourages reuse of coefficients.~\remark{More detail on this in supplement}
...
and having the model automatically induce and extend the pattern.
...
Similarity metric in program space:\\
\includegraphics[width = 8in]{figures/similarity.png}

Similarity metric in image space:\\
\includegraphics[width = 8in]{figures/imageSimilarity.png}
...
\begin{equation}
\hat{T}(I) = \argmax_{T\in \mathcal{F}(I)} L(I | \text{render}(T)) \times\probability_{\beta} [ \text{program}(T)] 
\end{equation}
...
\begin{equation}
  \beta^* = \argmax_{\beta} \expect \left[ \log \frac{\probability_{\beta} [\text{program}(T)]\times L(I|\text{render}(T))}{\sum_{T'\in \mathcal{F}(I)} \probability_{\beta} [\text{program}(T')]\times L(I|\text{render}(T'))} \right]
\end{equation}
