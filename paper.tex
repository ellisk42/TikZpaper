...
\begin{equation}
\hat{T}(I) = \argmax_{T\in \mathcal{F}(I)} L(I | \text{render}(T)) \times\probability_{\theta_p} [ \text{program}(T)] 
\end{equation}
where $\probability_{\theta_p} [\cdot]$ is a prior probability
distribution over programs parameterized by $\theta_p$.
This is equivalent to doing
MAP inference in a generative model where the program is first drawn
from $\probability_{\theta_p} [\cdot]$, then the program is executed deterministically,
...
we find a maximum likelihood estimate of $\theta_p$:
\begin{equation}
  \theta_p^* = \argmax_{\theta_p} \expect \left[ \log \frac{\probability_{\theta_p} [\text{program}(T)]\times L(I|\text{render}(T))}{\sum_{T'\in \mathcal{F}(I)} \probability_{\theta_p} [\text{program}(T')]\times L(I|\text{render}(T'))} \right]
\end{equation}
where the expectation is taken both over the model predictions and the
$(I,T)$ pairs in the training corpus.  We define $\probability_{\theta_p}
[\cdot]$ to be a log linear distribution $\propto \exp
(\theta\cdot \phi(\text{program}))$, where $\phi(\cdot)$ is a feature
