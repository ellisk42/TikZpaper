...
\end{equation}
where $|T|$ is the length of execution trace $T$, and the subscripts
...
We trained the network on $10^5$ scenes, which takes a little less than a day on a Nvidia TitanX GPU.
...
Concretely, we implemented the following scheme: for an image $I$, the neurally guided sampling scheme of section~\ref{neuralNetworkSection} samples a set of candidate traces, written $\mathcal{F}(I)$.
...
it modestly improves our Top-1 accuracy from 58\% to 60\%.  Recall that
