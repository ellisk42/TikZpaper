...
\author{Kevin Ellis$^{1,*}$\qquad Daniel Ritchie$^2$\qquad  Armando Solar-Lezama$^1$\qquad Joshua B. Tenenbaum$^1$\\
...
but the grid could be made arbitrarily fine. Appendix~\ref{architectureDetails} gives full architectural detail.  %% See supplement for the full details
...
\begin{figure}
  \includegraphics[width = \textwidth]{figures/policyComparison_basic_DC+bias_20.png}
    \caption{Time to synthesize a minimum cost program (compare w/ Table~\ref{policyEvaluation}). Sketch: out-of-the-box performance of Sketch~\citep{solar2008program}. DC: Deep--Coder style baseline that predicts program components, trained like~\cite{BalGauBroetal16}. Oracle:  upper bounds  the performance of any bias--optimal search policy. End--to--End: not shown because it times out on 96\% of drawings, and has its median time (63s) calculated only on non-timeouts, where is the other comparisons include timeouts in their median calculation. $\infty = $ timeout. Red dashed line is median time.}
  \end{figure}
